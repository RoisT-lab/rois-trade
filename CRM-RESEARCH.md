...
La encuesta y la simplificación del CRM ya están publicadas. La recuperación de enlaces está implementada en el worktree `dashboard-brand-refresh` y pendiente de publicación web. Los cambios aditivos `research_named_invitations` y `research_copy_links` ya se aplicaron a Supabase; SQL reproducible en `supabase-research-named-invitations.sql` y `supabase-research-copy-links.sql` (CLI local no disponible; ejecución mediante la conexión Supabase). No se reemplazaron enlaces de usuarios existentes. Las pruebas se revierten íntegramente: no envían correos ni conservan contactos o respuestas de prueba.
...
3. Generar enlaces ES/EN. **Copiar enlace** en el registro del CRM y en Seguimiento de invitaciones recupera el mismo enlace durante su vigencia de 30 días, incluso tras actualizar. La copia no rota el token ni amplía la caducidad. No se envía automáticamente: tratarlo como una invitación privada. Los enlaces históricos solo tienen hash y no se pueden reconstruir; la interfaz ofrece reemplazarlos con confirmación explícita y sin duplicar el contacto. El anterior deja de funcionar. Las encuestas respondidas no se reinician.
...
- `research_invitations`: vínculo único al CRM, hash SHA-256 de un token aleatorio, vigencia, creador y fecha de respuesta. Los tokens de nuevos enlaces se conservan aparte en `rois_research_private.invitation_links`, sin permisos directos para clientes y con RLS de denegación explícita. Solo un RPC con verificación de administrador aprobado devuelve un enlace individual; no se exponen en consultas CRM, listados, notas ni almacenamiento del navegador. Es una capacidad de respuesta, no de lectura; requiere el mismo cuidado que un enlace privado.
...
- Las escrituras privilegiadas viven en esquema privado, con `search_path` fijo; los RPC públicos son `SECURITY INVOKER`. Los permisos de administrador se consultan en perfiles, nunca en metadata editable por el usuario.
...
- `tests/research-copy-links.sql`: copia estable, caducidad sin extensión, permisos privados, reparación explícita de enlaces antiguos/vencidos, sin duplicación CRM ni reinicio de respuestas. Todo se revierte. El navegador comprueba copiar desde registros, reapertura, cancelar/confirmar reemplazo y alternativa manual si el portapapeles se bloquea.
