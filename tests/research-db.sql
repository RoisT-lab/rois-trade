...
begin
  inv := public.rois_create_research_invitation(null,'ROIS QA Research - rollback only','research-qa-'||gen_random_uuid()::text||'@example.invalid');
  perform set_config('research.qa.token',inv->>'token',true);
  perform set_config('research.qa.crm',inv->>'crm_id',true);
  again := public.rois_create_research_invitation((inv->>'crm_id')::uuid,'','');
...
begin
  ...
  begin perform public.rois_create_research_invitation(null,'QA','qa@example.invalid');raise exception 'FAIL: anon can create';exception when insufficient_privilege then null;end;
...
do $$ begin
  ...
  begin perform public.rois_create_research_invitation(null,'QA','qa@example.invalid');raise exception 'FAIL: non-admin create';exception when insufficient_privilege then null;end;
