...
(async()=>{
 ...
 try{
  ...
  await page.locator('#research-invite [name=email]').fill('qa@example.invalid');
  await page.route('**/rest/v1/rpc/rois_create_research_invitation',route=>route.fulfill({contentType:'application/json',body:JSON.stringify({token:'b'.repeat(64),crm_id:'qa-crm'})}));
