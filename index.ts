...
function messageMarkup(value = "") {
  const lines = String(value || "Tienes una nueva alerta pendiente en tu perfil ROIS.")
    .replace(/\r\n?/g, "\n")
    .replace(/\s*\u2022\s*/g, "\n- ")
    .split("\n");
  const blocks: string[] = [];
  ...
  const flushParagraph = () => {
    ...
    blocks.push(`<p style="font-size:16px;line-height:1.7;color:#3d4650;margin:0">${escapeHtml(paragraph.join(" "))}</p>`);
    paragraph = [];
  };
  const flushList = () => {
    if (!listItems.length) return;
    blocks.push(`<${listType} style="font-size:16px;line-height:1.65;color:#3d4650;margin:0;padding-left:24px">${listItems.map(item => `<li style="margin:0 0 7px">${escapeHtml(item)}</li>`).join("")}</${listType}>`);
    listType = "";
    listItems = [];
  };
  ...
  flushParagraph();
  flushList();
  return `<div style="display:grid;gap:14px;margin:0 0 26px">${blocks.join("")}</div>`;
}
...
function emailMarkup(item: QueueItem, appUrl: string) {
  ...
  const message = messageMarkup(item.message || "");
