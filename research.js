(function (root) {
  ...
  async function copyInvitation(options) {
    const lang=options.language==='en'?'en':'es', L=(es,en)=>text(lang,es,en);
    document.getElementById('research-link-dialog')?.remove();
    const dialog=document.createElement('dialog');
    dialog.id='research-link-dialog';dialog.className='research-admin research-link-dialog';
    dialog.setAttribute('aria-labelledby','research-link-title');
    dialog.setAttribute('data-no-translate','');dialog.setAttribute('translate','no');
    dialog.innerHTML=`<h3 id="research-link-title">${L('Copiar enlace de encuesta','Copy survey link')}</h3><p>${e(options.name||'')}</p><div role="status">${L('Recuperando enlace…','Retrieving link…')}</div><div class="research-links"></div><button type="button" data-replace-link hidden>${L('Crear enlace de reemplazo','Create replacement link')}</button><button type="button" data-close-link>${L('Cerrar','Close')}</button>`;
    document.body.append(dialog);
    dialog.querySelector('[data-close-link]').onclick=()=>dialog.close();
    dialog.onclose=()=>dialog.remove();dialog.showModal();
    const status=dialog.querySelector('[role=status]'),replace=dialog.querySelector('[data-replace-link]');
    async function load(repair=false) {
      replace.hidden=true;replace.disabled=true;
      try {
        if(!options.token||options.demo)throw Error('admin_required');
        const result=await request('rpc/rois_get_research_invitation_link',{p_crm_id:options.crmId,p_replace:repair},options.token);
        if(!dialog.isConnected)return;
        if(!/^[a-f0-9]{64}$/.test(result?.token||''))throw Error('invalid_response');
        dialog.querySelector('.research-links').innerHTML=['es','en'].map(l=>{
          const url=new URL('./encuesta.html',location.href);url.searchParams.set('lang',l);url.hash='token='+result.token;
          return `<label>${l==='es'?'Español':'English'}<input readonly value="${e(url.href)}"></label><button type="button" data-copy-link="${l}">${L('Copiar enlace','Copy link')} ${l.toUpperCase()}</button>`;
        }).join('');
        dialog.querySelectorAll('[data-copy-link]').forEach(b=>b.onclick=async()=>{
          const input=b.previousElementSibling.querySelector('input');
          try{await navigator.clipboard.writeText(input.value);status.textContent=L('Enlace copiado.','Link copied.');}
          catch{input.focus();input.select();status.textContent=L('Selecciona y copia el enlace mostrado.','Select and copy the displayed link.');}
        });
        status.textContent=result.answered?L('Esta encuesta ya fue respondida. El enlace no permite otra respuesta.','This survey was already answered. The link does not allow another response.'):L('Puedes copiar este mismo enlace cuando lo necesites. No se ha enviado ningún mensaje.','You can copy this same link whenever needed. No message has been sent.');
      }catch(err){
        if(!dialog.isConnected)return;
        if(/link_unavailable/.test(err.message)){
          status.textContent=L('El enlace anterior no se guardó para recuperación o ya venció. Puedes reemplazarlo; el enlace anterior dejará de funcionar.','The previous link was not saved for recovery or has expired. You can replace it; the previous link will stop working.');
          replace.hidden=false;replace.disabled=false;
        }else status.textContent=tError(err,lang);
      }
    }
    replace.onclick=()=>{if(confirm(L('¿Crear un enlace de reemplazo? El anterior dejará de funcionar. No se duplicará el contacto ni se borrarán respuestas.','Create a replacement link? The previous link will stop working. The contact will not be duplicated and no answers will be deleted.')))load(true);};
    await load();
  }
  ...
  async function mountAdmin(options) {
    ...
    form.elements.crm_id.onchange=()=>{const existing=!!form.elements.crm_id.value;form.querySelectorAll('[data-new-company]').forEach(el=>el.hidden=existing);form.elements.company.required=!existing;form.elements.recipient_name.required=!existing;if(existing){form.elements.recipient_name.value=invitations.find(i=>i.crm_id===form.elements.crm_id.value)?.recipient_name||companies.find(c=>c.id===form.elements.crm_id.value)?.name||'';}form.querySelector('button[type=submit]').textContent=existing?L('Obtener enlace','Get link'):L('Generar enlaces','Generate links');};
    form.onsubmit=async event=>{event.preventDefault();if(form.elements.crm_id.value){await copyInvitation({crmId:form.elements.crm_id.value,language:lang,token,name:form.elements.recipient_name.value});return;}const button=form.querySelector('button[type=submit]');if(button.disabled)return;button.disabled=true;const status=form.querySelector('[role=status]');status.textContent=L('Generando…','Generating…');form.querySelector('.research-links').replaceChildren();try{
      ...
      status.textContent=L('Enlaces listos. Puedes volver a copiarlos desde el registro del CRM. No se envió ningún correo.','Links ready. You can copy them again from the CRM record. No email was sent.');
    }catch(err){status.textContent=tError(err,lang);}finally{button.disabled=false;}};
  }
  root.ROISResearch={adminMarkup,mountAdmin,reportHtml,copyInvitation};
