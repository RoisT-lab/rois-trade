(function (root) {
  ...
  async function mountAdmin(options) {
    ...
    try {
      [records,invitations,companies]=await Promise.all([loadAll('research_responses'),loadAll('research_invitations','select=id,crm_id,created_at,expires_at,responded_at'),loadAll('crm','select=id,name,email,organization&prospect_type=eq.company')]);
    ...
    const L=(es,en)=>text(lang,es,en), selects=['country','sector','funding_type','funding_recency','category','budget','urgency'];
    host.querySelector('#research-admin-content').innerHTML=`<details><summary>${L('Crear invitación / enlace ES–EN','Create invitation / ES–EN link')}</summary><form id="research-invite"><div class="research-toolbar"><label>${L('Empresa del CRM','CRM company')}<select name="crm_id"><option value="">${L('Crear nuevo registro de investigación','Create new research record')}</option>${companies.sort((a,b)=>a.name.localeCompare(b.name)).map(c=>`<option value="${e(c.id)}">${e(c.name)} · ${e(c.email||'')}</option>`).join('')}</select></label><label data-new-company>${L('Nueva empresa','New company')}<input name="company" maxlength="180" required></label><label data-new-company>${L('Correo de contacto','Contact email')}<input name="email" type="email" maxlength="254" required></label><button type="submit">${L('Generar enlaces','Generate links')}</button></div><p>${L('Vigencia: 30 días. Regenerar un enlace pendiente invalida el anterior. Una respuesta por empresa invitada en esta versión.','Valid for 30 days. Regenerating a pending link invalidates the old one. One response per invited company for this version.')}</p><div class="research-message" role="status"></div><div class="research-links"></div></form></details><div class="research-toolbar"><button type="button" id="research-refresh">${L('Actualizar respuestas','Refresh responses')}</button><button type="button" id="research-pdf">${L('Exportar selección a PDF','Export selection to PDF')}</button><label><span>${L('Incluir respuestas completas','Include full responses')}</span><input type="checkbox" id="research-full" checked></label></div><div class="research-filters"><label>${L('Buscar empresa o respuesta','Search company or answer')}<input id="research-search" type="search"></label>${selects.map(name=>{const f=S.fields.find(f=>f.name===name);const values=[...new Set(records.map(r=>r.answers[name]))].filter(Boolean).sort();return `<label>${e(f[lang])}<select data-research-filter="${name}"><option value="">${L('Todos','All')}</option>${values.map(v=>`<option value="${e(v)}">${e(S.valueLabel(f,v,lang))}</option>`).join('')}</select></label>`;}).join('')}<label>${L('Idioma de respuesta','Response language')}<select data-research-filter="language"><option value="">${L('Todos','All')}</option><option value="es">Español</option><option value="en">English</option></select></label><label>${L('Respondida desde','Answered from')}<input type="date" id="research-from"></label><label>${L('Respondida hasta','Answered through')}<input type="date" id="research-to"></label></div><div id="research-results"></div><details><summary>${L('Seguimiento de invitaciones','Invitation tracking')} (${invitations.length})</summary><div class="research-table-wrap"><table><thead><tr><th>${L('Empresa','Company')}</th><th>${L('Estado','Status')}</th><th>${L('Vence','Expires')}</th></tr></thead><tbody>${invitations.map(i=>`<tr><td>${e(companies.find(c=>c.id===i.crm_id)?.name||i.crm_id)}</td><td>${i.responded_at?L('Respondida','Answered'):new Date(i.expires_at)<new Date()?L('Vencida','Expired'):L('Enlace generado; envío no verificado','Link generated; delivery not verified')}</td><td>${e(i.expires_at.slice(0,10))}</td></tr>`).join('')}</tbody></table></div></details>`;
    let filtered=[], pageIndex=0;
    const filterDescription=()=>[...host.querySelectorAll('[data-research-filter],#research-search,#research-from,#research-to')].filter(el=>el.value).map(el=>`${el.closest('label').firstChild.textContent.trim()}: ${el.tagName==='SELECT'?el.selectedOptions[0].textContent:el.value}`).join(' | ')||L('Sin filtros','No filters');
    function draw(reset=true){
      if(reset)pageIndex=0;
      const search=host.querySelector('#research-search').value.trim().toLocaleLowerCase(),from=host.querySelector('#research-from').value,to=host.querySelector('#research-to').value;
      filtered=records.filter(r=>(!search||Object.values(r.answers).join(' ').toLocaleLowerCase().includes(search))&&(!from||r.created_at.slice(0,10)>=from)&&(!to||r.created_at.slice(0,10)<=to)&&[...host.querySelectorAll('[data-research-filter]')].every(el=>!el.value||(el.dataset.researchFilter==='language'?r.language:r.answers[el.dataset.researchFilter])===el.value)).sort((a,b)=>b.created_at.localeCompare(a.created_at));
    ...
    form.elements.crm_id.onchange=()=>{const existing=!!form.elements.crm_id.value;form.querySelectorAll('[data-new-company]').forEach(el=>el.hidden=existing);form.elements.company.required=!existing;form.elements.email.required=!existing;};
    form.onsubmit=async event=>{event.preventDefault();const button=form.querySelector('button[type=submit]');if(button.disabled)return;button.disabled=true;const status=form.querySelector('[role=status]');status.textContent=L('Generando…','Generating…');form.querySelector('.research-links').replaceChildren();try{
      const result=await request('rpc/rois_create_research_invitation',{p_crm_id:form.elements.crm_id.value||null,p_company:form.elements.company.value.trim(),p_email:form.elements.email.value.trim()},token);
