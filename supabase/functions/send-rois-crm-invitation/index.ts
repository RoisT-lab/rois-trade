...
Deno.serve(async request => {
  ...
  try {
    ...
    // Agents must pass caller RLS before service-role email delivery.
    if (String(profile.role) === "commercial") {
      const { data: authorized, error: accessError } = await userClient
        .from("crm").select("id").eq("id", prospectId).maybeSingle();
      if (accessError || !authorized) return json({ error: "Account assignment is not authorized" }, 403);
    }
